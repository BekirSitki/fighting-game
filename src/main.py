...
# Fighter Data
WARRIOR_STARTING_POSITION = (SCREEN_WIDTH / 3)
WARRIOR_SIZE = 162
WARRIOR_SCALE = 4
WARRIOR_OFFSET = [72, 46]
WARRIOR_DATA = [WARRIOR_SIZE, WARRIOR_SCALE, WARRIOR_OFFSET]
WIZARD_STARTING_POSITION = (SCREEN_WIDTH / 3) * 2
WIZARD_SIZE = 250
WIZARD_SCALE = 3
...
def main_menu():
    ...
    while True:
        ...
        controls_button_y = SCREEN_HEIGHT // 2 + (button_height + button_spacing) * 0.5 + 50
...
def controls_screen():
    while True:
        ...
        draw_text(
            ...
            menu_font_title,
            RED,
            SCREEN_WIDTH // 2 - menu_font_title.size(controls_title)[0] // 2,
            50,
        )

        controls_font = pygame.font.Font("assets/fonts/turok.ttf", 40)
        shadow_offset = 3
        ...
        p1_x = SCREEN_WIDTH // 4 - 100
        p1_y = 200
        for i, line in enumerate(p1_controls):
            line_x = p1_x
            line_y = p1_y + i * 50
            draw_text(line, controls_font, BLACK, line_x + shadow_offset, line_y + shadow_offset)  # Shadow
            draw_text(line, controls_font, BLUE, line_x, line_y)  # Text
        ...
        p2_x = SCREEN_WIDTH * 3 // 4 - 200
        p2_y = 200
        for i, line in enumerate(p2_controls):
            line_x = p2_x
            line_y = p2_y + i * 50
            draw_text(line, controls_font, BLACK, line_x + shadow_offset, line_y + shadow_offset)  # Shadow
            draw_text(line, controls_font, RED, line_x, line_y)  # Text

        return_button = draw_button(
            "RETURN TO MAIN MENU", menu_font, BLACK, GREEN, SCREEN_WIDTH // 2 - 220, 700, 500, 50
        )
...
def reset_game():
    ...
    fighter_1 = Fighter(1, WARRIOR_STARTING_POSITION, 310, False, WARRIOR_DATA, warrior_sheet, WARRIOR_ANIMATION_STEPS, sword_fx)
    fighter_2 = Fighter(2, WIZARD_STARTING_POSITION, 310, True, WIZARD_DATA, wizard_sheet, WIZARD_ANIMATION_STEPS, magic_fx)
